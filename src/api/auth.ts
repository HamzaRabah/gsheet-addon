...
export class Auth {
  public static isApaleoApp() {
    const authType = PropertiesService.getScriptProperties().getProperty("AUTH_TYPE");

    return authType === "authorization_code";
  }

  static getApaleoAuthService() {
    const scriptProperties = PropertiesService.getScriptProperties();
    const userProperties = PropertiesService.getUserProperties();

    const properties = Auth.isApaleoApp() ? scriptProperties : userProperties;
  ...
  static isOAuth2ServiceWithPrivateApi(service: GoogleAppsScriptOAuth2.OAuth2Service): service is OAuth2ServiceWithPrivateApi {
